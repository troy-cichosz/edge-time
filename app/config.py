...
class Settings(BaseSettings):
    ...
    state_dir: str = "/var/lib/edge-time"
    gnss_observation_file: str = "/var/lib/edge-time/gnss.json"
    rtc_observation_file: str = "/var/lib/edge-time/rtc.json"
    ntp_host: str = "pool.ntp.org"
    ntp_port: int = 123
    poll_seconds: int = 5
    attestation_key: str = "/var/lib/edge-time/identity.key"
    http_timeout_seconds: float = 2.0

    @property
    def priorities(self) -> List[str]:
        return [x.strip() for x in self.source_priority.split(",") if x.strip()]
    ...
    def effective_gnss_url(self) -> str:
        ...
        return f"http://{self.effective_gnss_host}:{self.gnss_port}{path}"
