...
@app.post("/time/context")
def time_context():
    """
    Create one evidence-facing Capture Time Context.

    This is intentionally POST because context creation creates a new
    signed, hash-chained attestation record. Evidence services should
    request it at the capture event and retain the returned context with
    the artifact they are creating.
    """
    return engine.capture_context().model_dump(mode="json")


@app.get("/time/consistency")
def consistency():
    return engine.consistency_response()
